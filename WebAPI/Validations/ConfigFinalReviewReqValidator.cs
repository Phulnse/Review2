...
{
    public class ConfigFinalReviewReqValidator : AbstractValidator<ConfigFinalReviewReq>
    {
        public ConfigFinalReviewReqValidator(IUnitOfWork unitOfWork)
        {
            RuleFor(x => x.TopicId).MustAsync(async (id, _) =>
            {
                return await unitOfWork.Topic.IsValidTopicAsync(id, TopicStateEnum.FinaltermReport, TopicProgressEnum.WaitingForConfigureConference);
            });
